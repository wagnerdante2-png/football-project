...
import { scoutingReport, scoutingState } from '../src/scouting';
...
const radarStarted=Date.now(),radar=globalScoutingSearch(world,{observerClubId:observer.id,limit:40}),radarMs=Date.now()-radarStarted;
if(radar.scanned<30000||radar.matched<25000||radar.reports.length!==40)throw new Error(`Bounded world radar coverage failed: scanned=${radar.scanned} matched=${radar.matched} reports=${radar.reports.length}`);
if(radar.profiled>160)throw new Error(`World radar profiled too many identities: ${radar.profiled}`);
...
const knowledge=scoutingState(world).knowledge.size;
if(knowledge>1000)throw new Error(`Passive global discovery inflated persistent scouting knowledge: ${knowledge}`);
console.log(`[smoke-people-bundle] source=${manifest.sources.openfootball.snapshotCommit} · bundle=${players.length} · imported=${real.length} · adopted=${report.players.adoptedRuntime} · background=${pool.backgroundReal} · canonicalBRA=OK · radar=${radar.reports.length}/${radar.profiled}/${radar.matched} · knowledge=${knowledge} · search=OK · duplicates=0 · loadMs=${elapsed} · radarMs=${radarMs} · searchMs=${searchMs}`);
