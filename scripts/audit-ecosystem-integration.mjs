...
const activeEntries=[];for(const m of html.matchAll(/<script[^>]+type=['"]module['"][^>]+src=['"]([^'"]+)['"]/g)){const src=m[1].replace(/^\//,'').replace(/^src\//,'').replace(/\.(ts|tsx)$/,'');if(byModule.has(src))activeEntries.push(src)}
const activeReachable=reachable(activeEntries);

const rng=[];const snapshots=new Set(),restores=new Set();const persistentModules=[];
for(const [key,{text,file}] of byModule){const randomCount=(text.match(/Math\.random\s*\(/g)||[]).length;if(randomCount)rng.push({file:`src/${key}${file.endsWith('.tsx')?'.tsx':'.ts'}`,count:randomCount});const snap=[...text.matchAll(/export\s+function\s+(snapshot[A-Z][A-Za-z0-9_]*)\s*\(/g)].map(m=>m[1]);const restore=[...text.matchAll(/export\s+function\s+(restore[A-Z][A-Za-z0-9_]*)\s*\(/g)].map(m=>m[1]);for(const name of snap)snapshots.add(name.slice('snapshot'.length));for(const name of restore)restores.add(name.slice('restore'.length));if(snap.length&&restore.length)persistentModules.push({module:key,snapshotFunctions:snap,restoreFunctions:restore,reachableFromDaily:dailyReachable.has(key),reachableFromActiveApp:activeReachable.has(key)});}
rng.sort((a,b)=>b.count-a.count||a.file.localeCompare(b.file));
const rngCalls=rng.reduce((a,x)=>a+x.count,0);
const maxDirectRandomCalls=11;
...
console.log(`Repeated tick functions: ${repeated.length}`);for(const [name,count] of repeated)console.log(`  ${name} x${count} | ${allowedRepeatedTicks.get(name)??'UNREGISTERED'}`);
console.log(`Active legacy-named imports: ${legacyImports.length}`);for(const name of legacyImports)console.log(`  ${name} | ${allowedActiveLegacy.get(name)??'UNREGISTERED'}`);
console.log(`Source files using Math.random(): ${rng.length} (${rngCalls} calls; ceiling=${maxDirectRandomCalls})`);for(const x of rng.slice(0,25))console.log(`  ${x.file}: ${x.count}`);
console.log(`Snapshot families without matching restore name: ${snapshotWithoutRestore.length}`);for(const x of snapshotWithoutRestore)console.log(`  snapshot${x}`);
console.log(`Restore families without matching snapshot name: ${restoreWithoutSnapshot.length}`);for(const x of restoreWithoutSnapshot)console.log(`  restore${x}`);
console.log(`Active HTML entrypoints audited: ${activeEntries.length}; reachable modules: ${activeReachable.size}`);
console.log(`Transitive daily stateful modules with snapshot/restore: ${transitivePersistentModules.length}`);for(const x of transitivePersistentModules)console.log(`  DAILY ${x.module}: ${x.persisted?'PERSISTED':'MISSING SAVE PATH'} | ${x.snapshotFunctions.join(', ')}`);
console.log(`Active-app stateful modules with snapshot/restore: ${activePersistentModules.length}`);for(const x of activePersistentModules)console.log(`  ACTIVE ${x.module}: ${x.persisted?'PERSISTED':'MISSING SAVE PATH'} | ${x.snapshotFunctions.join(', ')}`);
...
fs.mkdirSync(path.join(root,'tmp'),{recursive:true});fs.writeFileSync(path.join(root,'tmp/ecosystem-integration-audit.json'),JSON.stringify(report,null,2));
