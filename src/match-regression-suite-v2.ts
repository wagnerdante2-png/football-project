...
function oneMatch(name:string,home:Club,away:Club,seed:number):RegressionCase{const s=createMatchCore(home,away,{seed});simulateMatchCore(s,home,away);const inv=validateMatchState(s),off=validateOfficiating(s),life=lifecyclePhysicsDiagnostics(s),evt=validateMatchEventAttribution(s,home,away),issues=[...inv,...off.issues,...life.issues,...evt.issues],ok=issues.length===0;return{name,ok,score:ok?100:Math.max(0,100-issues.length*12),issues,metrics:{homeGoals:s.home.score,awayGoals:s.away.score,homeXg:Number(s.home.xg.toFixed(2)),awayXg:Number(s.away.xg.toFixed(2)),seconds:Math.round(s.seconds)}}}
