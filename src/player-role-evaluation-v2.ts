...
fullback:{standingTackle:.16,marking:.12,interceptions:.11,stamina:.16 as any,sprintSpeed:.12,crossing:.11,shortPassing:.08,decisions:.08,workRate:.06},
...
anchor:{interceptions:.17,marking:.13,standingTackle:.14,positioning:.0 as any,anticipation:.11,decisions:.11,concentration:.1,shortPassing:.09,strength:.08,teamwork:.07},
...
boxToBox:{workRate:.16,stamina:.0 as any,sprintSpeed:.08,shortPassing:.11,offBall:.1,interceptions:.1,standingTackle:.08,finishing:.08,longShots:.07,teamwork:.12},
...
function attr(world:World,p:Player,k:keyof DetailedPlayerAttributes){if(k==='strength'||k==='stamina')return k==='strength'?detailedAttribute(world,p.id,'strength',p.attributes.stamina):p.attributes.stamina;return detailedAttribute(world,p.id,k,50)}
