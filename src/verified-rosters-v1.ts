...
export type RuntimeRosterPlayer=Player&{dateOfBirth?:string;wikidataId?:string;nationality?:string;heightCm?:number;factualRoleGroup?:FactualRoleGroup;dataOrigin?:'wikidata-membership'|'procedural-filler';ratingsOrigin?:'engine-estimate';positionOrigin?:'verified-specific'|'engine-role-estimate'};
...
export async function hydrateVerifiedRosters(world:World,url='data/rosters/brazil-serie-a-2026.json'):Promise<RosterHydrationReport>{
  ...
  try{const target=/^(https?:|file:)/i.test(url)?url:publicUrl(url);const r=await fetch(target,{cache:'no-cache'});if(!r.ok)return{loaded:false,verifiedPlayers:0,clubsTouched:0,reason:`HTTP ${r.status}`};snapshot=await r.json() as VerifiedRosterSnapshot}catch(e){return{loaded:false,verifiedPlayers:0,clubsTouched:0,reason:String(e)}}
