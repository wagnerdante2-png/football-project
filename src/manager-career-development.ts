...
function managerFor(world:World,clubId:string){const m=managerByClub(world,clubId);return m&&m.currentClubId===clubId?m:undefined;}

function processClubEvent(world:World,event:WorldEvent,clubId:string):void{
  const m=managerFor(world,clubId);if(!m)return;
  ...
  if(event.type==='SeasonEnded'){
    m.experienceYears+=1;rep(m,'coachingPrestige',.4);rep(m,'domestic',.22);
...
function processEvent(world:World,event:WorldEvent):void{
  const clubs=event.type==='SeasonEnded'?world.clubs.map(c=>c.id):[...new Set(event.clubIds)];
  for(const clubId of clubs)processClubEvent(world,event,clubId);
}
...
export function annualManagerAging(world:World,date:string):void{
  for(const club of world.clubs){const m=managerByClub(world,club.id);if(!m)continue;const currentYear=Number(date.slice(0,4));const birthYear=Number(m.dateOfBirth.slice(0,4));if(Number.isFinite(currentYear)&&Number.isFinite(birthYear))m.age=Math.max(18,currentYear-birthYear);}
}
