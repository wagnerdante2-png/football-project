...
import { matchArchive, playerCareerStats, snapshotMatchHistory } from './match-season-history-v2';
...
export function worldConsistencyDiagnosticsV2():WorldConsistencyReport{
 ...
 const hist1=snapshotMatchHistory(world),processed1=hist1.processed.length,matches1=hist1.matches.length;for(let i=0;i<fixtures.length;i++){const f=fixtures[i],s=states[i],home=world.clubs.find(c=>c.id===f.home)!,away=world.clubs.find(c=>c.id===f.away)!;const mod=require('./match-season-history-v2') as typeof import('./match-season-history-v2');mod.persistCompletedMatchV2(world,s,home,away,{fixtureId:`${world.season}:${f.round}:${f.home}:${f.away}`,competitionId:'consistency-league',date:'2026-08-24',round:f.round})}const hist2=snapshotMatchHistory(world);duplicateGuardOk=hist2.processed.length===processed1&&hist2.matches.length===matches1;
