...
export function playerVisual(player:Pick<Player,'name'> & {dateOfBirth?:string}):PlayerVisualIdentity|undefined{const candidates=state.manifest?.players.filter(x=>norm(x.name)===norm(player.name))??[];if(player.dateOfBirth){const exact=candidates.find(x=>x.dateOfBirth===player.dateOfBirth);if(exact)return exact}return candidates.length===1?candidates[0]:undefined}
