...
/** Resolve public assets relative to the document, not the filesystem root.
 * This is required by the portable Electron build where index.html is loaded via file://.
 */
export function publicAssetUrl(path:string){const clean=path.replace(/^\/+/, '');return new URL(clean,document.baseURI).toString()}
export async function loadVisualAssetManifest(base='data/visual/visual-assets-2026.json'){if(state.manifest)return state.manifest;if(state.loading)return state.loading;const url=/^(https?:|file:)/i.test(base)?base:publicAssetUrl(base);state.loading=fetch(url).then(async r=>{if(!r.ok)return;const m=await r.json() as VisualAssetManifest;state.manifest=m;return m}).catch(()=>undefined);return state.loading}
...
function resolvedAssetUrl(value:string){if(!value)return value;if(/^(https?:|data:|blob:|file:)/i.test(value))return value;return publicAssetUrl(value)}
export function crestMarkup(name:string,className='visual-crest'){const v=clubVisual(name);if(v.crest?.url){const src=resolvedAssetUrl(v.crest.thumbnailUrl??v.crest.url);return `<img class="${className}" src="${esc(src)}" alt="Escudo ${esc(name)}" loading="eager" decoding="async" referrerpolicy="no-referrer">`}const initials=name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase();return `<span class="${className} procedural-crest" style="--vc1:${v.primaryColor};--vc2:${v.secondaryColor}">${esc(initials||'FC')}</span>`}
export function playerFaceMarkup(player:Pick<Player,'name'> & {dateOfBirth?:string},className='player-face'){const v=playerVisual(player);if(v?.portrait?.url){const src=resolvedAssetUrl(v.portrait.thumbnailUrl??v.portrait.url);return `<img class="${className}" src="${esc(src)}" alt="${esc(player.name)}" loading="lazy" decoding="async" referrerpolicy="no-referrer">`}return `<span class="${className} procedural-face" aria-hidden="true"><i></i></span>`}
