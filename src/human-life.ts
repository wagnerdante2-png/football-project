...
export type LifeEventType='familyIllness'|'relationshipConflict'|'relationshipSupport'|'pregnancy'|'paternityLeave'|'birth'|'nightOut'|'lateNight'|'fanEncounter'|'charityEvent'|'mediaAppearance'|'relationshipRumour'|'publicRelationship'|'breakup'|'friendConflict'|'friendSupport'|'bereavement'|'familyVisit'|'socialMediaPraise'|'socialMediaBacklash'|'publicIncident';
...
export type LifePerson={
  id:string; kind:PersonKind; name:string; clubId?:string; fame:number; mediaInterest:number; privacy:number; temperament:number; professionalism:number; sociability:number;
};
export type Relationship={
  id:string; aId:string; bId:string; type:RelationshipType; closeness:number; stability:number; publicVisibility:number; startedDate:string; lastChangedDate:string; notes:string[];
};
export type PersonalLoad={
  concentration:number; morale:number; fatigue:number; availability:boolean; performancePct:number; mediaPressure:number; socialSupport:number;
};
export type LifeEvent={
  id:string; type:LifeEventType; personId:string; relatedPersonIds:string[]; clubIds:string[]; startDate:string; endDate:string; status:LifeEventStatus; severity:number; publicity:number; truth:'confirmed'|'rumour'|'false';
  headline:string; description:string; choiceRequired:boolean; options?:LifeChoiceOption[]; chosenOptionId?:string; effects:PersonalLoad; tags:string[];
};
...
export type LifeProfile={
  personId:string; familyOrientation:number; nightlifeAffinity:number; fameComfort:number; scandalRisk:number; relationshipNeed:number; stressResilience:number; currentLoad:PersonalLoad; publicProfile:PublicProfile;
};
...
function playerPerson(p:Player):LifePerson{
  const s=hash(`life-person-${p.id}`);return{id:p.id,kind:'player',name:p.name,clubId:p.clubId,fame:clamp(p.currentAbility*.72+p.potentialAbility*.18+p.age<23?4:0),mediaInterest:Math.round(25+p.currentAbility*.45+pseudo(s,1)*25),privacy:Math.round(25+pseudo(s,2)*70),temperament:Math.round(20+pseudo(s,3)*78),professionalism:Math.round(35+pseudo(s,4)*60),sociability:Math.round(25+pseudo(s,5)*70)};
}
...
function ensureRelationship(world:World,p:Player,date:string):Relationship|undefined{
  const s=humanLifeState(world);const profile=s.profiles.get(p.id)!;if(relationBetween(s,p.id,`partner-${p.id}`))return undefined;
  const chance=(profile.relationshipNeed+profile.sociability)/2500;if(Math.random()>chance)return undefined;
  const roll=Math.random();const kind:PersonKind=roll<.18?'celebrity':roll<.33?'footballer':'partner';const partner=externalPerson(`partner-${p.id}-${date}`,kind,kind==='footballer'&&Math.random()<.5?world.clubs[Math.floor(Math.random()*world.clubs.length)]?.id:undefined);s.people.set(partner.id,partner);
...
function eventEffect(type:LifeEventType,severity:number):PersonalLoad{
  ...
  if(type==='relationshipConflict'||type==='breakup'){e.concentration=-2-severity*.65;e.morale=-3-severity*.8;e.performancePct=100-(1+severity*.6);e.mediaPressure=severity*.7;}
  if(type==='relationshipSupport'||type==='friendSupport'||type==='familyVisit'){e.morale=2+severity*.5;e.performancePct=100+Math.min(2,severity*.2);e.socialSupport=3+severity*.6;}
  ...
  if(type==='mediaAppearance'||type==='fanEncounter'||type==='charityEvent'){e.morale=1+severity*.2;e.mediaPressure=severity*.3;}
  if(type==='relationshipRumour'||type==='socialMediaBacklash'||type==='publicIncident'){e.concentration=-1-severity*.5;e.mediaPressure=3+severity;e.performancePct=100-Math.min(4,severity*.45);}
...
function combineLoad(events:LifeEvent[]):PersonalLoad{
  const x=baseLoad();for(const ev of events){x.concentration+=ev.effects.concentration;x.morale+=ev.effects.morale;x.fatigue+=ev.effects.fatigue;x.mediaPressure+=ev.effects.mediaPressure;x.socialSupport+=ev.effects.socialSupport;x.availability=x.availability&&ev.effects.availability;x.performancePct*=ev.effects.performancePct/100;}
  x.performancePct=clamp(x.performancePct,82,104);return x;
}
function createEvent(world:World,p:Player,date:string,type:LifeEventType,severity:number,duration:number,publicity:number,headline:string,description:string,relatedPersonIds:string[]=[]):LifeEvent{
  const s=humanLifeState(world);const ev:LifeEvent={id:`life-${world.season}-${hash(`${p.id}-${date}-${type}-${Math.random()}`)}`,type,personId:p.id,relatedPersonIds,clubIds:[p.clubId],startDate:date,endDate:addDays(date,duration),status:'active',severity,publicity,truth:type==='relationshipRumour'?(Math.random()<.55?'rumour':'false'):'confirmed',headline,description,choiceRequired:false,effects:eventEffect(type,severity),tags:[type]};s.events.push(ev);
  emitWorldEvent(world,{type:'PersonalLifeEvent',date,clubIds:[p.clubId],playerIds:[p.id],importance:severity>=8?4:severity>=5?3:2,summary:headline,payload:{lifeEventId:ev.id,type,severity,publicity,endDate:ev.endDate,description}});return ev;
}

function maybeDailyEvent(world:World,p:Player,date:string):void{
  const s=humanLifeState(world);const profile=s.profiles.get(p.id)!;const person=s.people.get(p.id)!;const active=s.events.some(e=>e.personId===p.id&&e.status==='active'&&e.endDate>=date);const base=.0045+(100-profile.stressResilience)/50000+person.mediaInterest/50000;if(Math.random()>base*(active?.55:1))return;
  ...
  const roll=Math.random();let type:LifeEventType;let sev=2+Math.round(Math.random()*6);let dur=2+Math.round(Math.random()*10);let pub=person.mediaInterest*.25;let headline='Evento pessoal';let desc='Um acontecimento fora do futebol afeta a rotina.';
  if(roll<.10){type='familyIllness';sev=4+Math.round(Math.random()*5);dur=5+Math.round(Math.random()*24);headline=`${p.name} vive preocupação familiar`;desc='Doença de familiar gera carga emocional temporária.';}
  else if(roll<.20&&partner){type='relationshipConflict';headline=`Momento delicado na vida pessoal de ${p.name}`;desc='Conflito no relacionamento aumenta estresse fora de campo.';pub=partnerPerson?.fame??10;}
  else if(roll<.28&&partner){type='relationshipSupport';headline=`Apoio pessoal fortalece ${p.name}`;desc='Relacionamento estável oferece suporte emocional.';}
  else if(roll<.34&&partner){type='pregnancy';dur=30;headline=`Família de ${p.name} espera um filho`;desc='Expectativa de nascimento altera agenda e estado emocional.';}
  else if(roll<.40){type='nightOut';sev=Math.round((profile.nightlifeAffinity+Math.random()*40)/18);dur=1;headline=`${p.name} é visto em evento noturno`;desc='Noite fora pode ser irrelevante ou gerar questionamentos dependendo do contexto.';pub=person.fame*.45;}
  else if(roll<.50){type='fanEncounter';dur=1;headline=`${p.name} encontra torcedores fora do clube`;desc='Abordagem espontânea de fãs pode gerar boa repercussão ou desgaste.';pub=person.fame*.5;}
  else if(roll<.58){type='charityEvent';dur=1;headline=`${p.name} participa de ação social`;desc='Participação pública reforça imagem positiva.';pub=person.fame*.55;}
  else if(roll<.68&&partnerPerson&&partnerPerson.fame>55){type='publicRelationship';dur=7;headline=`Relacionamento de ${p.name} ganha destaque`;desc='Relacionamento com pessoa conhecida amplia exposição pública de forma volátil.';pub=Math.max(person.fame,partnerPerson.fame);}
  else if(roll<.76){type='relationshipRumour';dur=4;headline=`Imprensa especula sobre vida pessoal de ${p.name}`;desc='Rumor sem confirmação circula em veículos e redes sociais.';pub=person.mediaInterest*.8;}
  else if(roll<.84){type='mediaAppearance';dur=1;headline=`${p.name} participa de evento de mídia`;desc='Exposição pública aumenta visibilidade temporária.';pub=person.fame*.7;}
  else if(roll<.93){type='friendSupport';dur=3;headline=`Amizades ajudam ${p.name} em semana importante`;desc='Rede social próxima reduz pressão emocional.';}
  ...
  createEvent(world,p,date,type,sev,dur,Math.round(clamp(pub)),headline,desc,partnerId?[partnerId]:[]);
}

function processScheduledFamily(world:World,p:Player,date:string):void{
  const s=humanLifeState(world);const pregnancy=s.events.find(e=>e.personId===p.id&&e.type==='pregnancy'&&e.status==='active');if(!pregnancy)return;
  const daysToEnd=Math.round((new Date(`${pregnancy.endDate}T12:00:00Z`).getTime()-new Date(`${date}T12:00:00Z`).getTime())/86400000);
...
export function tickHumanLife(world:World,date:string):void{
  const s=humanLifeState(world);if(s.lastDailyTick===date)return;s.lastDailyTick=date;
  for(const ev of s.events)if(ev.status==='active'&&date>ev.endDate)ev.status='resolved';
  for(const club of world.clubs)for(const p of club.players){ensureRelationship(world,p,date);processScheduledFamily(world,p,date);maybeDailyEvent(world,p,date);const active=s.events.filter(e=>e.personId===p.id&&e.status==='active'&&e.startDate<=date&&e.endDate>=date);const profile=s.profiles.get(p.id)!;profile.currentLoad=combineLoad(active);p.morale=Math.round(clamp(p.morale+profile.currentLoad.morale*.04-profile.currentLoad.mediaPressure*.01,20,100));}
}
