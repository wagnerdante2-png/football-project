...
export function simulateAdaptiveMatchCore(state:MatchCoreState,home:Club,away:Club,maxTicks=32000,knockout:KnockoutContext={requiresWinner:false}){const original={home:{pressing:home.tactics.pressing,width:home.tactics.width,defensiveLine:home.tactics.defensiveLine},away:{pressing:away.tactics.pressing,width:away.tactics.width,defensiveLine:away.tactics.defensiveLine}};try{for(let i=0;i<maxTicks&&state.phase!=='finished';i++){stepMatch(state,home,away,knockout);if(i%20===0)observe(state);if(i%80===0){applyAdaptive(home,state);applyAdaptive(away,state)}}syncMatchLifecycle(state);return state}finally{Object.assign(home.tactics,original.home);Object.assign(away.tactics,original.away)}}
