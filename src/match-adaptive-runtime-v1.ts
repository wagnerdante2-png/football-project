...
import { prepareMatchContext, stepMatchContext, maybeResolveHandball } from './match-context-bridge-v2';
import type { KnockoutContext } from './match-time-competition-v2';

const clamp=(v:number,a=0,b=100)=>Math.max(a,Math.min(b,v));
function contextRng(state:MatchCoreState){state.seed=(Math.imul(state.seed,1664525)+1013904223)>>>0;return state.seed/4294967296}
...
export function simulateAdaptiveMatchCore(state:MatchCoreState,home:Club,away:Club,maxTicks=24000,knockout:KnockoutContext={requiresWinner:false}){const original={home:{pressing:home.tactics.pressing,width:home.tactics.width,defensiveLine:home.tactics.defensiveLine},away:{pressing:away.tactics.pressing,width:away.tactics.width,defensiveLine:away.tactics.defensiveLine}};prepareMatchContext(state);try{for(let i=0;i<maxTicks&&state.phase!=='finished';i++){stepMatch(state,home,away,knockout);if((state.phase as string)!=='finished'){const rng=()=>contextRng(state);stepMatchContext(state,home,away,rng);maybeResolveHandball(state,home,away,rng)}if(i%20===0)observe(state);if(i%80===0){applyAdaptive(home,state);applyAdaptive(away,state)}}syncMatchLifecycle(state);return state}finally{Object.assign(home.tactics,original.home);Object.assign(away.tactics,original.away)}}
