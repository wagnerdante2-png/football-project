...
const src=(id:string,repo:string,path:string,kind:OpenFootballSource['kind'],countryId?:string,confederationId?:string):OpenFootballSource=>({id,repo,path,kind,countryId,confederationId,license:'CC0-1.0',historical:true});
...
export const GLOBAL_FOOTBALL_SOURCES={
 nationalAndInternational:src('of-global-leagues','openfootball/leagues','leagues.txt','leagues',undefined,undefined),
 southAmericaInternational:src('of-conmebol-leagues','openfootball/leagues','south-america/leagues.txt','leagues',undefined,'CONMEBOL'),
 europeInternational:src('of-uefa-leagues','openfootball/leagues','europe/leagues.txt','leagues',undefined,'UEFA')
};

export function sourceBundle(countryId:string){return FOOTBALL_SOURCE_CATALOG.find(x=>x.countryId===countryId)}
export function allCatalogSources(){return[GLOBAL_FOOTBALL_SOURCES.nationalAndInternational,GLOBAL_FOOTBALL_SOURCES.southAmericaInternational,GLOBAL_FOOTBALL_SOURCES.europeInternational,...FOOTBALL_SOURCE_CATALOG.flatMap(x=>[...x.leagueSources,...x.clubSources,...x.stadiumSources])]}
