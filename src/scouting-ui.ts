...
export type ScoutingUiState = {position?: Position;selectedPlayerId?: string;showShortlist: boolean};
...
export function renderScouting(world: World, observer: Club, state: ScoutingUiState): string {
  ...
  <div class="scouting-toolbar"><select id="scout-position"><option value="">Todas as posições</option>${positions.map(p=>`<option value="${p}" ${state.position===p?'selected':''}>${p}</option>`).join('')}</select><button id="toggle-shortlist" class="secondary">${state.showShortlist?'Ver candidatos':'Ver lista de interesse'}</button><span class="scouting-global-note">Pool global: ${num(pool.active)} ativos · ${num(pool.retired)} aposentados preservados</span></div>
  <div class="scouting-layout"><section class="card scouting-list"><div class="section-title"><h2>${state.showShortlist?'Lista de interesse':'Melhores candidatos do universo global'}</h2><span>${candidates.length} exibidos</span></div>${candidates.length?`<table><thead><tr><th>Jogador</th><th>Clube</th><th>Pos</th><th>Idade</th><th>CA estimado</th><th>PA estimado</th><th>Conf.</th><th></th></tr></thead><tbody>${candidates.map(r=>`<tr class="${selected?.playerId===r.playerId?'selected-scout':''}"><td><button class="scout-player" data-scout-player="${r.playerId}">${r.playerName}</button></td><td>${r.clubId==='unattached'?'Vínculo não mapeado':world.clubs.find(c=>c.id===r.clubId)?.name??r.clubId}</td><td>${r.position}</td><td>${r.age}</td><td>${range(r.currentAbility)}</td><td>${range(r.potentialAbility)}</td><td>${r.confidence}%</td><td><span class="scout-grade ${r.recommendation}">${recommendationLabel[r.recommendation]}</span></td></tr>`).join('')}</tbody></table>`:'<div class="empty-state">Nenhum jogador nesta seleção.</div>'}</section>
...
export function bindScouting(world: World, observer: Club, state: ScoutingUiState, rerender:()=>void): void {
  document.querySelector<HTMLSelectElement>('#scout-position')?.addEventListener('change',event=>{const value=(event.target as HTMLSelectElement).value;state.position=(value||undefined) as Position|undefined;state.selectedPlayerId=undefined;rerender()});
