...
export type TransferRecord = { season:number; playerId:string; playerName:string; fromClubId?:string; toClubId?:string; fee:number; weeklyWage:number; kind:'transfer'|'free'|'renewal'|'release'; };
export type EconomyState = { contracts:Map<string,Contract>; finances:Map<string,ClubFinance>; freeAgents:Player[]; transfers:TransferRecord[]; };
...
export function economyState(world:World):EconomyState{
  let state=economyByWorld.get(world); if(!state){state={contracts:new Map(),finances:new Map(),freeAgents:[],transfers:[]};economyByWorld.set(world,state);}
...
function recalcWages(world:World,state:EconomyState):void{for(const club of world.clubs){const f=state.finances.get(club.id)!;f.wageSpend=club.players.reduce((s,p)=>s+(state.contracts.get(p.id)?.weeklyWage??0),0);}}
...
function movePlayer(state:EconomyState,player:Player,from:Club|undefined,to:Club,season:number,fee:number,kind:TransferRecord['kind']):void{
  ...
  const wage=suggestedWage(player,to);state.contracts.set(player.id,{playerId:player.id,clubId:to.id,startSeason:season,endSeason:season+(player.age<=22?4:player.age>=32?2:3),weeklyWage:wage,squadStatus:statusFor(player,to)});
...
function handleContracts(world:World,state:EconomyState,season:number):void{
  for(const club of world.clubs)for(const player of [...club.players]){
    const contract=state.contracts.get(player.id);if(!contract||contract.endSeason>season)continue;const importance=statusFor(player,club);
...
function findPlayerAndSeller(world:World,playerId:string):{player:Player;seller:Club}|undefined{
  for(const seller of world.clubs){const player=seller.players.find(p=>p.id===playerId);if(player)return{player,seller};}return undefined;
}
...
function runTransferAI(world:World,state:EconomyState,season:number):void{
  ...
  for(const buyer of buyers){
    ...
    for(const {position,need} of needs){
      ...
      const reports=scoutingCandidates(world,buyer.id,position,24);
      const candidates:{player:Player;seller:Club;score:number;fee:number;confidence:number}[]=[];
      for(const report of reports){
        const found=findPlayerAndSeller(world,report.playerId); if(!found||found.seller.id===buyer.id)continue;
        const ca=(report.currentAbility.min+report.currentAbility.max)/2; const pa=(report.potentialAbility.min+report.potentialAbility.max)/2;
        const estimatedValue=(report.marketValue.min+report.marketValue.max)/2;
        const perceivedScore=ca+(pa-ca)*(report.age<=23?.5:.14)+(report.age<=21?4:0)+report.confidence*.045;
        if(report.confidence<34){if(perceivedScore>=72)assignScout(world,buyer.id,report.playerId);continue;}
        if(report.recommendation==='avoid'||report.recommendation==='monitor')continue;
        const sell=saleWillingness(found.seller,found.player);if(sell<-10)continue;
        const sellerAsk=playerMarketValue(found.player)*clamp(1.02+(20-sell)/100,.82,1.38);
        const fee=Math.round(Math.max(sellerAsk,estimatedValue*.9)/10000)*10000;
...
export function processOffseasonMarket(world:World,season:number):void{const state=economyState(world);handleContracts(world,state,season);signFreeAgents(world,state,season);runTransferAI(world,state,season);annualFinanceReset(world,state);recalcWages(world,state);}
