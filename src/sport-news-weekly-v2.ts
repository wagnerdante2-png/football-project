...
import { enrichSportNewsWithHistoricalMemory } from './sport-news-memory-runtime-v1';
...
function enrich(w:World,issue:SportNewsIssue){if(!(issue as any).__worldEnriched){const extras=coreArticles(w,issue.weekStart,issue.weekEnd),p1=extras.filter(x=>x.page===1),p2=extras.filter(x=>x.page===2),p3=extras.filter(x=>x.page===3);issue.pages[0].articles=[...p1,...issue.pages[0].articles].sort((a,b)=>b.importance-a.importance).slice(0,7);issue.pages[1].articles=[...p2,...issue.pages[1].articles].sort((a,b)=>b.importance-a.importance).slice(0,10);issue.pages[2].articles=[...p3,...issue.pages[2].articles].sort((a,b)=>b.importance-a.importance).slice(0,10);applyExtraEffects(w,extras);(issue as any).__worldEnriched=true}enrichSportNewsWithHistoricalMemory(w,issue);const all=issue.pages.flatMap(p=>p.articles),top=[...all].sort((a,b)=>b.importance-a.importance)[0];if(top)issue.coverHeadline=top.headline;issue.featuredClubIds=[...new Set(all.flatMap(a=>a.clubIds))].slice(0,16);issue.featuredPlayerIds=[...new Set(all.flatMap(a=>a.playerIds))].slice(0,16);issue.featuredManagerIds=[...new Set(all.flatMap(a=>a.managerIds))].slice(0,12);archiveSportNewsIssue(w,issue);return issue}
