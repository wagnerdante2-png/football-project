...
export function advanceOneDay(world:World):{date:string;matchDay:boolean;playedRound?:number}{
  const state=dailyCalendar(world);const date=state.date;institutionalState(world);tickPromises(world,date);tickTemporalProcesses(world,date);tickHumanLife(world,date);tickSocialWorld(world,date);tickDressingRoom(world,date);
  ...
  if(matchDay){
    playedRound=world.round;emitWorldEvent(world,{type:'MatchDayStarted',date,importance:2,summary:`Início da rodada ${playedRound}.`,payload:{round:playedRound}});playCurrentRoundWithMedical(world);tickScoutingRound(world);tickTemporalProcesses(world,date);tickDressingRoom(world,date);
  ...
  }else executeTrainingDay(world,date,trainingContext(world,date));
