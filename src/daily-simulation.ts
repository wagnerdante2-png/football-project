...
function defaultPlan(club:Club):DailyClubPlan{
  const intense=club.tactics.pressing>=72||club.tactics.tempo>=72;
  return{clubId:club.id,focus:intense?'recovery':'tactical',intensity:intense?'low':'medium',restDay:false};
}

function buildMatchDates(world:World,start:string):Map<number,string>{
  const rounds=[...new Set(world.fixtures.map(f=>f.round))].sort((a,b)=>a-b);const map=new Map<number,string>();
  rounds.forEach((round,i)=>map.set(round,addDays(start,i*7)));
  return map;
}

export function dailyCalendar(world:World):DailyCalendarState{
  let state=states.get(world);
  if(!state){const start=`${world.season}-07-25`;state={date:start,seasonStart:start,matchDates:buildMatchDates(world,start),clubPlans:new Map(),daysAdvanced:0};states.set(world,state);}
  for(const club of world.clubs)if(!state.clubPlans.has(club.id))state.clubPlans.set(club.id,defaultPlan(club));
  return state;
}

export function setClubTrainingPlan(world:World,clubId:string,patch:Partial<Omit<DailyClubPlan,'clubId'>>):void{
  const state=dailyCalendar(world);const current=state.clubPlans.get(clubId)??{clubId,focus:'tactical',intensity:'medium',restDay:false};state.clubPlans.set(clubId,{...current,...patch,clubId});
}

function trainPlayer(player:Player,plan:DailyClubPlan):void{
  if(plan.restDay){player.condition=Math.round(clamp(player.condition+7+Math.random()*5,35,100));return;}
  const intensity=plan.intensity==='high'?1.5:plan.intensity==='low'?.55:1;
  const cost=plan.focus==='recovery'?0:plan.intensity==='high'?6:plan.intensity==='medium'?3.5:1.5;
  const recovery=plan.focus==='recovery'?9:2.2;
  player.condition=Math.round(clamp(player.condition-cost+recovery+Math.random()*2,35,100));
  const ageFactor=player.age<=20?1.45:player.age<=24?1.2:player.age<=29?1:player.age<=33?.62:.34;
