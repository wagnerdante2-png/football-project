...
function feedClub(world:World,fixture:Fixture,clubId:string,starterIds:string[],date:string){
 ...
 for(let i=0;i<used.length;i++)for(let j=i+1;j<used.length;j++){const shared=Math.min(used[i].m,used[j].m);if(shared>=15)recordPartnershipMinutes(world,used[i].p,used[j].p,shared,{sharedSuccess});}
}
