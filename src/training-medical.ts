...
function weightedTrainingInjury(world:World,playerId:string){const candidates=injuryDefinitions.filter(d=>d.overloadBias>=.6&&d.baseSeverity!=='careerThreatening');const weights=candidates.map(d=>Math.max(.05,d.overloadBias));const total=weights.reduce((a,b)=>a+b,0);let roll=worldRandom(world,'world',`training-medical:injury:${playerId}`)*total;for(let i=0;i<candidates.length;i++){roll-=weights[i];if(roll<=0)return candidates[i];}return candidates[0];}
...
export function wireTrainingMedical(world:World):void{
  ...
  onWorldEvent(world,'TrainingOverloadWarning',(event)=>{
    ...
    const prevention=Math.max(.58,1-(medical-.45)*.5);const chance=Math.min(.62,(.12+pressure/120*proneness*(1.2-durability*.45))*prevention);if(worldRandom(world,'world',`training-medical:trigger:${playerId}`)>chance)return;
    const def=weightedTrainingInjury(world,playerId);if(!def)return;inflictInjury(world,player,def,worldRandom(world,'world',`training-medical:cause:${playerId}`)<.25?'recurrence':'overload');
  });
}
