...
export function eventBusState(world: World): EventBusState {
  let state=states.get(world);
  if(!state){state={events:[],nextSequence:1,listeners:new Map()};states.set(world,state);}
  return state;
}

export function onWorldEvent(world:World,type:WorldEventType|'*',listener:EventListener):()=>void{
  const state=eventBusState(world);let set=state.listeners.get(type);if(!set){set=new Set();state.listeners.set(type,set);}set.add(listener);
  return()=>set!.delete(listener);
}

export function emitWorldEvent(world:World,input:{type:WorldEventType;date?:string;actorIds?:string[];clubIds?:string[];playerIds?:string[];importance?:1|2|3|4|5;tags?:string[];summary:string;payload?:Record<string,unknown>}):WorldEvent{
  const state=eventBusState(world);const seq=state.nextSequence++;
  const event:WorldEvent={id:`evt-${world.season}-${seq}`,sequence:seq,type:input.type,date:input.date??`${world.season}-01-01`,season:world.season,round:world.round,actorIds:input.actorIds??[],clubIds:input.clubIds??[],playerIds:input.playerIds??[],importance:input.importance??2,tags:input.tags??[],summary:input.summary,payload:input.payload??{}};
  state.events.push(event);
  if(state.events.length>10000)state.events.splice(0,state.events.length-10000);
  for(const listener of state.listeners.get(event.type)??[])listener(event,world);
  for(const listener of state.listeners.get('*')??[])listener(event,world);
  return event;
}
