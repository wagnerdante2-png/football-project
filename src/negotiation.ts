...
function publicSentiment(world:World,buyer:Club,seller:Club|undefined,player:Player,fee:number,leak:LeakState,prefs:PlayerCareerPreferences):SentimentSnapshot{
  const s=negotiationState(world);const be=s.environments.get(buyer.id)!;const se=seller?s.environments.get(seller.id):undefined;const fame=player.currentAbility+(player.potentialAbility>=86?7:0)+(prefs.idolStatus>75?8:0);const value=playerMarketValue(player);const pricePenalty=Math.max(0,fee/Math.max(1,value)-1)*18;
  ...
  const supportersSeller= seller?clamp(48+prefs.idolStatus*.28+prefs.currentClubAttachment*.18-(prefs.wantsToLeave*.24)+(fame-seller.reputation)*.4+(rnd()-.5)*12,4,98):20;
  const mediaBuyer=clamp(40+(fame-buyer.reputation)*.78+(fee>value*1.25?-12:5)+(rnd()-.5)*15,5,96);
  const mediaSeller=seller?clamp(45+fame*.3+prefs.idolStatus*.2+(prefs.wantsToLeave>65?-8:8)+(rnd()-.5)*14,5,96):20;
...
function playerInterest(world:World,buyer:Club,seller:Club|undefined,player:Player,p:NegotiationPackage,prefs:PlayerCareerPreferences):number{
  const st=negotiationState(world);const be=st.environments.get(buyer.id)!;const se=seller?st.environments.get(seller.id):undefined;const climate=climateCompatibility(prefs.climatePreference,be.climate);const repGain=buyer.reputation-(seller?.reputation??55);const project=be.sportingProject;const role=roleValue(p.squadRole);const money=Math.min(100,p.weeklyWage/Math.max(1,player.currentAbility*80));const relocation=se&&se.region!==be.region?(prefs.relocationTolerance*.55+prefs.languageAdaptability*.25+climate*.2):90;const attachmentPenalty=seller?(prefs.currentClubAttachment*.18+prefs.idolStatus*.11):0;const leaveBonus=prefs.wantsToLeave*.24;
...
function sellerAcceptance(world:World,seller:Club|undefined,player:Player,p:NegotiationPackage,prefs:PlayerCareerPreferences,sent:SentimentSnapshot):number{
  if(!seller)return 100;const value=playerMarketValue(player);const total=p.upfrontFee+p.installments*p.installmentValue+(p.clauses.find(c=>c.type==='sellOn')?.value??0)*value/100;const scarcity=seller.players.filter(x=>x.position===player.position).length<=2?18:0;const starPenalty=player.currentAbility>=Math.max(...seller.players.map(x=>x.currentAbility))-3?20:0;const idolPenalty=prefs.idolStatus*.18;const desireBonus=prefs.wantsToLeave*.2;return clamp((total/Math.max(1,value))*58+desireBonus-starPenalty-scarcity-idolPenalty-sent.sellerPressure*.12,0,100);
}
...
export type StartNegotiationInput={fee:number;weeklyWage:number;transferBudget:number;initiatedBy?:NegotiationCase['initiatedBy'];urgency?:NegotiationCase['urgency'];dealType?:DealType;squadRole?:NegotiationPackage['squadRole'];};
export function negotiateTransfer(world:World,buyer:Club,seller:Club|undefined,player:Player,input:StartNegotiationInput):NegotiationCase{
  const st=negotiationState(world);const agent=st.agents.get(player.id)!;const prefs=st.preferences.get(player.id)!;const value=playerMarketValue(player);
  ...
  const sentiment=publicSentiment(world,buyer,seller,player,input.fee,'private',prefs);const c:NegotiationCase={id:`neg-${world.season}-${world.round}-${buyer.id}-${player.id}-${Math.floor(rnd()*1e6)}`,season:world.season,gameRound:world.round,buyerClubId:buyer.id,sellerClubId:seller?.id,playerId:player.id,playerName:player.name,status:'open',leakState:'private',initiatedBy:input.initiatedBy??'coach',urgency:input.urgency??'normal',agent,preferences:prefs,sentiment,rounds:[],currentPackage:initial,maxBuyerCost:Math.min(input.transferBudget,Math.round(value*(1.65+(buyer.reputation-player.currentAbility)/250))),sellerWalkAway:seller?Math.round(value*(.82+(100-prefs.wantsToLeave)/220+(prefs.idolStatus/300))):0,playerMinimumScore:Math.round(46+(100-prefs.relocationTolerance)*.12+(prefs.statusDrive*.08)),createdAtRound:world.round};
  st.cases.push(c);
  for(let r=1;r<=8&&c.status==='open';r++){
    c.sentiment=publicSentiment(world,buyer,seller,player,c.currentPackage.upfrontFee,c.leakState,prefs);
    const interest=playerInterest(world,buyer,seller,player,c.currentPackage,prefs);const sellerScore=sellerAcceptance(world,seller,player,c.currentPackage,prefs,c.sentiment);const buyScore=buyerValue(world,buyer,player,c.currentPackage);const totalCost=c.currentPackage.upfrontFee+c.currentPackage.installments*c.currentPackage.installmentValue+c.currentPackage.signingBonus+c.currentPackage.agentFee;const buyerLeverage=clamp((100-sellerScore)*.35+prefs.wantsToLeave*.3+(seller&&seller.players.length>27?10:0),0,100);const sellerLeverage=clamp((100-interest)*.2+prefs.idolStatus*.25+c.sentiment.sellerPressure*.3+(sellerScore<45?12:0),0,100);
