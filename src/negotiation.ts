...
function playerInterest(world:World,buyer:Club,seller:Club|undefined,player:Player,p:NegotiationPackage,prefs:PlayerCareerPreferences,externalSeller=false):number{
  const st=negotiationState(world);const be=st.environments.get(buyer.id)!;const se=seller?st.environments.get(seller.id):undefined;const climate=climateCompatibility(prefs.climatePreference,be.climate);const repGain=buyer.reputation-(seller?.reputation??55);const project=be.sportingProject;const role=roleValue(p.squadRole);const wageRatio=Math.max(.25,p.weeklyWage/Math.max(1,player.currentAbility*80)),money=clamp(50+Math.log2(wageRatio)*18,5,100),bonus=clamp(p.signingBonus/Math.max(1,p.weeklyWage*8)*100,0,100),personalTerms=money*.78+bonus*.22;const relocation=se&&se.region!==be.region?(prefs.relocationTolerance*.55+prefs.languageAdaptability*.25+climate*.2):externalSeller?75:90;const attachmentPenalty=(seller||externalSeller)?(prefs.currentClubAttachment*.18+prefs.idolStatus*.11):0;const leaveBonus=prefs.wantsToLeave*.24;
  return clamp(project*.16+role*.18+personalTerms*(prefs.moneyMotivation/100)*.2+(50+repGain*1.8)*(prefs.ambition/100)*.2+relocation*.15+prefs.statusDrive*.08+leaveBonus-attachmentPenalty,0,100);
}
