...
import { sportNewsIssues, type SportNewsCategory, type SportNewsIssue } from './sport-news-weekly-v1';
...
function syncArchive(w:World){archiveSportNewsIssues(w,sportNewsIssues(w,520));return state(w)}
...
export function snapshotSportNewsArchive(w:World):SportNewsArchiveSnapshot{return{entries:JSON.parse(JSON.stringify(syncArchive(w)))}}
