 ...
 *
 * The mirror is deferred to a microtask so click handlers that schedule the
 * canonical engine renderer can enqueue first. Document-side decorators then
 * observe the final view instead of a placeholder that will be overwritten.
 */
for(const type of bridgedEvents){
  window.addEventListener(type,event=>{
    if(event.target!==window)return;
    const detail=(event as CustomEvent).detail;
    queueMicrotask(()=>document.dispatchEvent(new CustomEvent(type,{detail})));
  });
}
